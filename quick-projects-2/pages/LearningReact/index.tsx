...
import { FolderInfo } from '@/services/getLearningPagesFolders';
...
import { IoArrowDown, IoArrowUp, IoSearchSharp, IoSwapVertical } from 'react-icons/io5';
import { DateTime } from 'luxon';
import { TbClockEdit } from 'react-icons/tb';
import { LuFolderPlus } from 'react-icons/lu';
import { MdMemory } from 'react-icons/md';
import { formatBytes } from '@/services/helpers';
import { getFolders } from '@/services/server-scripts/getFolders';
import { ISort } from '@/interfaces';

export const getStaticProps: GetStaticProps = async () => {
  return { props: { folders: getFolders('pages/LearningReact') } };
};

const storageKey = 'learningReactSort';
const initialSort: ISort = { prop: 'name', order: 'asc' };

export default function LearningReact({ folders }: { folders: FolderInfo[] }): React.JSX.Element {
  ...
  return (
    <div className={styles.main}>
      ...
      <div className={styles.toolbar}>
        ...
        <Input
          size='small'
          placeholder='Search learning page...'
          value={search}
...
const PagesList = ({ folders }: PagesListProps): React.JSX.Element => {
  ...
  const FolderPage = ({ folder }: { folder: FolderInfo }): React.JSX.Element => {
    ...
    const onClick = (): void => { router.push(`/LearningReact/${folder.name}`); };
