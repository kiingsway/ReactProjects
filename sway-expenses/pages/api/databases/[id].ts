...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const data = await Promise.all(pages.map(expandRelations));
  ...
  if (req.method === "POST") {
    try {
      ...
      if (!payload.properties) return res.status(400).json({ error: "Missing properties in body" });
