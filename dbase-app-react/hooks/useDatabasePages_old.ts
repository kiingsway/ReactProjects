...
import { getPageUrl, getRelationArrayFromProperties, removeDuplicates } from "@/app/services/helpers";
...
export function useDatabasePages(): IUseDBPages {
  ...
  useEffect(() => {
    const loadData = async (): Promise<void> => setData(await getRelationValues(dataSWR));
    if (dataSWR) loadData();
  }, [dataSWR]);
